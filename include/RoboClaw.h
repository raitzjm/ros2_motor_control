...
#define SERIAL_OBJ 	serial::Serial
...
public:
	// public methods
	RoboClaw(SERIAL_OBJ *hserial,uint32_t tout);
	...
	bool ReadCurrents(uint8_t address, int16_t &current1);
	...
	void begin(long speed, const char* device);
	bool isListening();
	bool overflow();
	int peek();
	virtual int read();
